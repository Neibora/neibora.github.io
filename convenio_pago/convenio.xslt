...
<xsl:stylesheet version="1.0"
  ...
  xmlns:state="http://panax.io/state">
	...
	<xsl:template match="/Convenio">
		...
		<xsl:variable name="totalConvenio">
			<xsl:call-template name="sum-values">
				<xsl:with-param name="nodes" select="Deuda/Concepto/@convenio"/>
			</xsl:call-template>
		</xsl:variable>
		<xsl:variable name="totalConvenioN" select="number($totalConvenio)"/>
				...
				<div class="p-8 md:p-12">
					...
					<div class="mb-8">
						<h1 class="text-2xl font-bold text-primary dark:text-white border-b-2 border-accent inline-block pb-1 mb-4">
							<div>At'n. <xsl:value-of select="Propietario/@nombre"/></div>
							<div><xsl:value-of select="Propietario/@domicilio"/></div>
						</h1>
							...
							<table class="w-full text-sm text-left">
								<thead class="bg-slate-50 dark:bg-slate-800 text-primary dark:text-accent uppercase text-xs font-bold">
									<tr>
										...
										<th class="px-6 py-4 text-right">Adeudo original</th>
										<th class="px-6 py-4 text-right">Convenio</th>
								...
								<tbody class="divide-y divide-slate-200 dark:divide-slate-800">
									<xsl:for-each select="Deuda/Concepto">
										<tr>
											...
											<td class="px-6 py-3 text-right font-medium font-mono">
												<xsl:call-template name="money">
													<xsl:with-param name="v" select="@convenio"/>
												</xsl:call-template>
											</td>
											<td class="px-6 py-3 text-right font-medium font-mono">
												<xsl:choose>
													<xsl:when test="number(@monto) &gt; 0 and string(@convenio) != '' and number(@convenio) &lt; number(@monto)">
														<xsl:variable name="descuentoPct"
															select="round(((number(@monto) - number(@convenio)) * 1000) div number(@monto)) div 10"/>
														<xsl:value-of select="format-number($descuentoPct, '0.0')"/>
														<xsl:text>%</xsl:text>
													</xsl:when>
								...
								<tfoot>
									<tr class="bg-green-50 dark:bg-emerald-950/40 border-t border-green-100 dark:border-emerald-900">
										<td class="px-6 py-4 text-xs font-bold text-primary dark:text-accent uppercase">
										...
										<td class="px-6 py-4 text-right">
											<p class="text-lg font-black text-primary dark:text-accent font-mono">
												<xsl:call-template name="money">
													<xsl:with-param name="v" select="$totalConvenioN"/>
												</xsl:call-template>
											</p>
										</td>
										<td class="px-6 py-4 text-right font-medium font-mono">
											<xsl:if test="$totalDeudaN &gt; 0 and $totalConvenioN &lt; $totalDeudaN">
												<xsl:variable name="descuentoTotalPct"
													select="round((($totalDeudaN - $totalConvenioN) * 1000) div $totalDeudaN) div 10"/>
												<xsl:value-of select="format-number($descuentoTotalPct, '0.0')"/>
												<xsl:text>%</xsl:text>
											</xsl:if>
										</td>
									</tr>
						...
						<div class="space-y-4 text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
							...
							<div class="space-y-3">
								<xsl:for-each select="Pagos/Pago">
									<p>
										<strong>
											<xsl:value-of select="position()"/>. <xsl:value-of select="@titulo"/>:
										</strong>
