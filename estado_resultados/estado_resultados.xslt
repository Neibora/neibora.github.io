...
<xsl:stylesheet version="1.0"
  xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/">
		<html lang="es">
			<head>
				...
				<title>
					Estado de resultados · <xsl:value-of select="estado-resultados/@desarrollo"/>
				</title>
				<style>
					:root{
			...
			<body>
				<header class="appbar">
					<div class="appbrand">
						<img src="../assets/img/logo.png" alt="Neibora"/>Neibora · Estado de resultados
					</div>
					<div class="appmeta">
						<strong>
							<xsl:value-of select="estado-resultados/@desarrollo"/>
						</strong>Información financiera mensual
					</div>
				</header>
				<div class="controls">
						...
						<select id="periodPicker">
							<xsl:for-each select="estado-resultados/periodo">
								<option value="{@id}">
									...
									<xsl:call-template name="period-label">
					...
					<div class="view-switch" aria-label="Modo de visualización">
						<button type="button" id="singleMode">Periodo individual</button>
						<button type="button" id="compareMode" class="active">Comparar periodos</button>
					</div>
				</div>
				...
				<script>
					...
					var details=document.querySelectorAll('.drill-row[data-detail-id="'+id+'"]');
					var open=!line.classList.contains('open');
					details.forEach(function(detail){detail.classList.toggle('open',open);});
	...
	<xsl:template match="periodo">
		...
		<xsl:variable name="recovered" select="number(concat('0',@cuotas-recuperadas))"/>
		<xsl:variable name="advanced" select="number(concat('0',@cuotas-adelantadas))"/>
		<xsl:variable name="ordinaryQuota" select="$quota - $recovered - $advanced"/>
		<xsl:variable name="extraordinary" select="egresos/partida/movimiento[translate(@tipo,'ABCDEFGHIJKLMNOPQRSTUVWXYZÁÉÍÓÚ','abcdefghijklmnopqrstuvwxyzáéíóú')='extraordinario'][@origen='Interno']"/>
		<section class="period-sheet" data-period="{@id}">
			<header class="sheet-header">
				<div class="sheet-logo">
					<img src="../assets/img/logo.png" alt="Neibora"/>Neibora
				</div>
				<div class="sheet-title">
					<h1>
						Estado de resultados <xsl:call-template name="period-label">
							<xsl:with-param name="id" select="@id"/>
						</xsl:call-template>
					</h1>
					<strong>
				...
				<xsl:if test="@estatus='parcial'">
					<span class="period-chip">Parcial</span>
				</xsl:if>
			</header>
			...
			<div class="block">
				<table class="sheet-table assessment">
					<tr>
						<th colspan="2">CUOTA DE MANTENIMIENTO MENSUAL POR CASA</th>
						<td class="amount">
							<xsl:call-template name="money">
								<xsl:with-param name="value" select="@cuota-mensual"/>
							</xsl:call-template>
						</td>
					</tr>
					<tr>
						<td class="count">
							<xsl:value-of select="@casas"/>
						</td>
						<th>CUOTAS COBRABLES</th>
						<td class="amount">
							<xsl:call-template name="money">
								<xsl:with-param name="value" select="@cuotas-cobrables"/>
							</xsl:call-template>
						</td>
					</tr>
				</table>
			...
			<div class="block">
				<table class="sheet-table result-table">
					<tr>
						<td class="label">REMANENTE PERIODO ANTERIOR</td>
						<td></td>
						<td class="amount green">
							<xsl:call-template name="money">
								<xsl:with-param name="value" select="@remanente-anterior"/>
							</xsl:call-template>
						</td>
					</tr>
					<tr class="spacer">
					...
					<xsl:call-template name="income-row">
						<xsl:with-param name="label" select="'INGRESOS CUOTAS DE MANTENIMIENTO'"/>
						<xsl:with-param name="value" select="$ordinaryQuota"/>
						<xsl:with-param name="code" select="'cuota'"/>
					</xsl:call-template>
					<tr>
						<td>INGRESOS CUOTAS DE MANTENIMIENTO ADELANTADAS</td>
						<td></td>
						<td class="amount">
							<xsl:call-template name="money">
								<xsl:with-param name="value" select="$advanced"/>
							</xsl:call-template>
						</td>
					</tr>
					<tr>
						<td>INGRESOS RECUPERACIÓN CUOTAS ATRASADAS</td>
						<td></td>
						<td class="amount">
							<xsl:call-template name="money">
								<xsl:with-param name="value" select="$recovered"/>
							</xsl:call-template>
						</td>
					</tr>
					<xsl:call-template name="income-row">
						<xsl:with-param name="label" select="'INGRESOS POR RECARGOS'"/>
						<xsl:with-param name="value" select="sum(ingresos/partida[@codigo='recargo']/@monto)"/>
						<xsl:with-param name="code" select="'recargo'"/>
					</xsl:call-template>
					<xsl:call-template name="income-row">
						<xsl:with-param name="label" select="'INGRESOS CUOTAS EXTRAORDINARIAS INTERNAS'"/>
						<xsl:with-param name="value" select="sum(ingresos/partida[@codigo='cuota-interna']/@monto)"/>
						<xsl:with-param name="code" select="'cuota-interna'"/>
					</xsl:call-template>
					<xsl:call-template name="income-row">
						<xsl:with-param name="label" select="'INGRESOS CUOTAS EXTRAORDINARIAS EXTERNAS'"/>
						<xsl:with-param name="value" select="sum(ingresos/partida[@codigo='cuota-externa']/@monto)"/>
						<xsl:with-param name="code" select="'cuota-externa'"/>
					</xsl:call-template>
					<xsl:call-template name="income-row">
						<xsl:with-param name="label" select="'INGRESOS POR RENTA DE AMENIDADES'"/>
						<xsl:with-param name="value" select="sum(ingresos/partida[@codigo='renta']/@monto)"/>
						<xsl:with-param name="code" select="'renta'"/>
					</xsl:call-template>
					<xsl:call-template name="income-row">
						<xsl:with-param name="label" select="'INGRESOS MULTAS'"/>
						<xsl:with-param name="value" select="sum(ingresos/partida[@codigo='multa']/@monto)"/>
						<xsl:with-param name="code" select="'multa'"/>
					</xsl:call-template>
					<xsl:call-template name="income-row">
						<xsl:with-param name="label" select="'INGRESO TAGS'"/>
						<xsl:with-param name="value" select="sum(ingresos/partida[@codigo='tag']/@monto)"/>
						<xsl:with-param name="code" select="'tag'"/>
					</xsl:call-template>
					<xsl:call-template name="income-row">
						<xsl:with-param name="label" select="'DEPÓSITOS NO IDENTIFICADOS'"/>
						<xsl:with-param name="value" select="sum(ingresos/partida[@codigo='sin-identificar']/@monto)"/>
						<xsl:with-param name="code" select="'sin-identificar'"/>
					</xsl:call-template>
					<tr class="total-row">
						<td colspan="2">TOTAL DE INGRESOS</td>
						<td class="amount">
							<xsl:call-template name="money">
								<xsl:with-param name="value" select="ingresos/@total"/>
							</xsl:call-template>
						</td>
					</tr>
					<tr>
						<td colspan="2">EGRESOS ORDINARIOS</td>
						<td class="amount">
							<xsl:call-template name="money">
								<xsl:with-param name="value" select="egresos/@ordinarios"/>
							</xsl:call-template>
						</td>
					</tr>
					<tr>
						<td colspan="2">EGRESOS EXTRAORDINARIOS</td>
						<td class="amount">
							<xsl:call-template name="money">
								<xsl:with-param name="value" select="egresos/@extraordinarios"/>
							</xsl:call-template>
						</td>
					</tr>
					<tr class="total-row">
						<td colspan="2">TOTAL EGRESOS DEL MES</td>
						<td class="amount">
							<xsl:call-template name="money">
								<xsl:with-param name="value" select="egresos/@total"/>
							</xsl:call-template>
						</td>
					</tr>
					<tr class="ending">
						<td colspan="2">REMANENTE PERIODO ACTUAL</td>
						<td>
							<xsl:attribute name="class">
								amount<xsl:if test="@saldo-final &lt; 0"> negative</xsl:if>
							</xsl:attribute>
							<xsl:call-template name="money">
								<xsl:with-param name="value" select="@saldo-final"/>
							</xsl:call-template>
						</td>
					</tr>
				</table>
			...
			<table class="sheet-table expense-table">
				<thead>
					<tr>
						...
						<th class="amount">Internos</th>
						<th class="amount">Externos</th>
						<th class="amount">Total</th>
					</tr>
				</thead>
				<tbody>
					<xsl:apply-templates select="egresos/partida" mode="expense"/>
				</tbody>
				<tfoot>
					<tr class="grand-total">
						...
						<td class="amount">
							<xsl:call-template name="money">
								<xsl:with-param name="value" select="sum(egresos/partida/@interno)"/>
							</xsl:call-template>
						</td>
						<td class="amount">
							<xsl:call-template name="money">
								<xsl:with-param name="value" select="sum(egresos/partida/@externo)"/>
							</xsl:call-template>
						</td>
						<td class="amount">
							<xsl:call-template name="money">
				...
				<tbody>
					<xsl:choose>
						<xsl:when test="$extraordinary">
							<xsl:for-each select="$extraordinary">
								<tr>
									<td class="index">
						...
						<xsl:otherwise>
							<tr>
								<td></td>
								<td class="empty-note">Sin gastos extraordinarios en el periodo</td>
			...
			<div class="sheet-footer">
				Fuente consolidada · <xsl:value-of select="/estado-resultados/@generado"/>
			</div>
		</section>
	</xsl:template>
	...
	<xsl:template name="income-row">
		...
		<tr>
			<xsl:attribute name="class">
				income-line<xsl:if test="$item/movimiento"> has-detail</xsl:if>
			</xsl:attribute>
		...
		<xsl:if test="$item/movimiento">
			<tr class="drill-row" id="{$detailId}" data-detail-id="{$detailId}">
				<td colspan="3">
					<table class="drill-table">
						<tbody>
							<xsl:for-each select="$item/movimiento">
								<tr>
	...
	<xsl:template match="partida" mode="expense">
		...
		<tr>
			<xsl:attribute name="class">
				expense-line<xsl:if test="movimiento"> has-detail</xsl:if>
			</xsl:attribute>
			<xsl:if test="movimiento">
				<xsl:attribute name="data-detail">
					<xsl:value-of select="$detailId"/>
				</xsl:attribute>
			</xsl:if>
			...
			<td class="amount">
				<xsl:call-template name="money">
					<xsl:with-param name="value" select="@interno"/>
				</xsl:call-template>
			</td>
			<td class="amount">
				<xsl:call-template name="money">
					<xsl:with-param name="value" select="@externo"/>
				</xsl:call-template>
			</td>
			<td class="amount">
				<strong>
		...
		<xsl:if test="movimiento">
			<xsl:for-each select="movimiento">
				<tr class="drill-row expense-detail" data-detail-id="{$detailId}">
					<td></td>
					<td class="detail-concept">
						<xsl:if test="@fecha">
	...
	<xsl:template name="money">
		<xsl:param name="value"/>
		<xsl:choose>
			<xsl:when test="string($value)='' or number($value)!=number($value)">$0.00</xsl:when>
			<xsl:when test="number($value) &lt; 0">
				(<xsl:text>$</xsl:text><xsl:value-of select="format-number(0-number($value),'#,##0.00','mx')"/>)
			</xsl:when>
			<xsl:otherwise>
				<xsl:text>$</xsl:text>
				<xsl:value-of select="format-number(number($value),'#,##0.00','mx')"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
</xsl:stylesheet>
