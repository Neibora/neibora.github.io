...
(function () {
        ...
        for (const filter of filters) {
            ...
            function wireOption(chk, text) {
                ...
                lb.addEventListener('click', (ev) => {
                    ...
                    domain.querySelectorAll('input[type="checkbox"]:checked').forEach(cb => {
    ...
    xo.listener.on(['loteador:mostrarNumerosDeCasa::map'], async function () {
        const map = this;
        let container = map.parentElement;
        if (!container) return;
        //container = container.querySelector('.map-labels') || container.appendChild(document.createElement('div'));
        //container.classList.add("map-labels");
        ...
        for (const item of items) {
            ...
            Object.assign(label.style, { position: 'absolute', left: `${x}px`, top: `${y}px`, transform: 'translate(-50%, -50%)', fontWeight: 'bold', color: 'black', fontSize: '.8rem', pointerEvents: 'none', textShadow: '0 1px 2px rgba(255,255,255,.8)' });
